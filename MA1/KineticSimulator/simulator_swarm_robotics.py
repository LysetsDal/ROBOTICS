    ...
    def robot_controller(self, swarm_mode):
        ...
        if (swarm_mode == 1):
          ...
          if too_close or self.broadcast_signal:
              ...
              if all(r["distance"] > PROX_SENSOR_RANGE * 0.8 for r in self.prox_readings):
                  self.broadcast_signal = False
          else:
              self.set_rotation_and_speed(0, MAX_SPEED * 0.5)
...
def snapshot_metrics(robots, total_time, last_metric_time, interval=5.0):
    ...
    hull_area = 0.0
    if len(positions) >= 3:
        hull_area = float(ConvexHull(positions).volume)  # use .volume for 3D, .area for 2D hull perimeter/area

    # Print metrics
    print(f"[t={total_time:.1f}s] Avg NN distance: {avg_nn:.2f}, Hull area: {hull_area:.2f}")
...
def main():
    ...
    while running:
        ...
        last_metric_time = snapshot_metrics(robots, total_time, last_metric_time, interval=5.0)
